...
export function ProveedoresManager() {
  ...
  const fetchContactosParaSelect = async (proveedorId: string) => {
    try {
      const { data, error } = await supabase
        .from('contactos')
        .select('id, nombre')
  ...
  const handleOpenModal = async (proveedor?: Proveedor) => {
    if (proveedor) {
      setFormData({
        id: proveedor.id,
        razon_social: proveedor.razon_social,
        cuit: proveedor.cuit || '',
        telefono_general: proveedor.telefono_general || '',
        email_general: proveedor.email_general || '',
        contacto_principal_id: proveedor.contacto_principal_id || '',
      });
      await fetchContactosParaSelect(proveedor.id);
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('success', 'Proveedor guardado exitosamente.');
      handleCloseModal();
      fetchData();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('¿Está seguro de que desea dar de baja este proveedor?')) return;
    try {
      const { error } = await supabase
        .from('proveedores')
        ...
        .eq('id', id);
      if (error) throw error;
      showToast('success', 'Proveedor dado de baja exitosamente.');
      fetchData();
    } catch (err: any) {
      console.error('Error deleting data:', err);
      showToast('error', 'Error al dar de baja el proveedor.');
    }
  };
              ...
              <tbody>
                {filteredProveedores.map((p) => (
                  <tr key={p.id} className="border-b hover:bg-gray-50">
                    <td className="p-4 font-medium text-gray-900">{p.razon_social}</td>
                    <td className="p-4 text-gray-600">{p.cuit || '-'}</td>
                    <td className="p-4 text-gray-600">{p.email_general || '-'}</td>
                    <td className="p-4 text-gray-600">{p.telefono_general || '-'}</td>
          ...
          <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col z-50 animate-in slide-in-from-right">
            ...
            <div className="flex border-b px-6 pt-2">
              ...
              <button
                ...
                title={!formData.id ? 'Guarda el proveedor primero para agregar contactos' : ''}
              >
                Contactos
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {activeTab === 'generales' ? (
                <form id="proveedorForm" onSubmit={handleSave} className="space-y-4">
                  <div>
                ...
                <div className="h-full">
                  {formData.id && <ContactosTab entidadTipo="proveedor" entidadId={formData.id} />}
                </div>
              )}
            </div>
