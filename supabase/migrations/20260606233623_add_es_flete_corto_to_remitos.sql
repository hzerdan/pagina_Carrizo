-- Agregar columna es_flete_corto a la tabla remitos si no existe
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'remitos' AND column_name = 'es_flete_corto') THEN
        ALTER TABLE "public"."remitos" ADD COLUMN "es_flete_corto" boolean DEFAULT false;
    END IF;
END $$;

-- Actualizar la función RPC para soportar el nuevo campo
